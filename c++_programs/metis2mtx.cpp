...
int main(int argc, char** argv){
	...
	for(long u = 0; u < g.t_vtx; u++)
	{
		for(unsigned int j = g.row_map[u]; j < g.row_map[u+1]; j++){
			unsigned int v = g.entries[j];
            if(u < v){
			    out_s << (v + 1) << " " << (u + 1) << std::endl;
            }
		}
	}
	std::ofstream out(argv[2]);
    out << "%%MatrixMarket matrix coordinate pattern symmetric" << std::endl;
	out << g.t_vtx << " " << g.t_vtx << " " << (g.nnz / 2) << std::endl;
	out << out_s.str();
	out.close();
	return 0;
}
